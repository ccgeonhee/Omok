...
void COMOKAI::fillWeight(int myStone, bool isRenju) 
{
	...
	int (COMOKAI::*fp[6])(int, int, int, int) = {&COMOKAI::IsTwo, &COMOKAI::IsOpenTwo, &COMOKAI::IsThree, &COMOKAI::IsOpenThree, &COMOKAI::IsFour, &COMOKAI::yourOpenFour};
...
void COMOKAI::play(int &x, int &y, int myStone)
{
	...
	for(int i = l.y; i <= r.y; i++)
	{
		...
		{
			if(!IsEmpty(j, i, myStone)) continue;
			if (findPutPoint(j, i, myStone, &COMOKAI::IsFive)) { x = j; y = i; return; }
			if(findPutPoint(j, i, myStone, &COMOKAI::IsSix))  {x = j; y = i; return;}
			if(findPutPoint(j, i, yourStone, &COMOKAI::IsFive))  {x = j; y = i; return;}
			if(findPutPoint(j, i, yourStone, &COMOKAI::IsSix))  {x = j; y = i; return;}
			if(findPutPoint(j, i, myStone, &COMOKAI::IsFourThree))  {x = j; y = i; return;}
			if(findPutPoint(j, i, myStone, &COMOKAI::IsDoubleFour))  {x = j; y = i; return;}
			//if(isOpenFour) break;
			if(findPutPoint(j, i, yourStone, &COMOKAI::IsFourThree)) {x = j; y = i; return;}
			if(findPutPoint(j, i, yourStone, &COMOKAI::IsDoubleFour)) {x = j; y = i; return;}
		}
	}
